...
function parseDate(date) {
	...
	if (type === 'date-time') {
		var fullTime = [addZero(d.getHours()), addZero(d.getMinutes()), addZero(d.getSeconds()), 'Z'].join(':');
...
Validator.prototype.swaggerValidate = function () {
	...
	if (schema && definition && Object.getOwnPropertyNames(propertyObj).length > 0 && field) {
		(function () {
			...
			Object.keys(propertyObj).forEach(function (item) {
				properties[item] = '';
			});
			...
			spec.validateModel(schema, definition, properties, function (err, result) {
